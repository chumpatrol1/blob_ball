...
class EnvironmentalModifiers:
    ...
    def update(self):
        ...
        if(self.species == 'glue_shot' and self.y_pos > 1350):
            create_environmental_modifier(player = self.player, affects = self.affects, species = 'glue_puddle_' + str(self.player), x_pos = self.x_pos - 27.5, y_pos = 1378, lifetime = 180)
            self.lifetime = 0
