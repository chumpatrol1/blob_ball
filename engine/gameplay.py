...
def handle_gameplay(p1_selected, p2_selected, ruleset, settings, p1_is_cpu, p2_is_cpu):
    ...
    if not initialized:
        ...
        if(timer == 0):
            if(p1_blob.is_cpu):
                cpu_logic = engine.cpu_logic.handle_logic(p1_blob, p2_blob, ball, game_score, timer)
                p1_blob.move(cpu_logic)
            else:
                p1_blob.move(pressed)
            if(p2_blob.is_cpu):
                cpu_logic = engine.cpu_logic.handle_logic(p2_blob, p1_blob, ball, game_score, timer)
